...
/**
 * POST /video/progress/:id_video
 * body: { seconds: number, completado?: boolean }
 * Protected: requireAuth + checkVideoAccess middleware
 * 
 ...
 * - SEGURIDAD: Verifica acceso al video antes de guardar progreso
 */
router.post('/progress/:id_video', checkVideoAccess, async function(req, res, next) {
  try {
    const user = req.session.user;
    const db = req.app.locals.db;
    const { id_video } = req.params; // Obtener de la URL
    const { seconds, completado } = req.body; // Obtener solo el resto del body
    const videoId = parseInt(id_video); // Mantener compatibilidad con lógica existente

    // Validación de parámetros
    if (!videoId || isNaN(videoId) || typeof seconds === 'undefined') {
      return res.status(400).json({ 
        success: false, 
        message: 'Parámetros inválidos: id_video válido y seconds son requeridos' 
      });
    }
