  ...
  try {
    ...
    const statsQuery = await pool.request().query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM Favoritos) / NULLIF(COUNT(DISTINCT id_usuario), 0) as promedioFavoritos
      FROM Favoritos
    `);
    
    // Obtener cursos más guardados con info de compras
    const cursosFavoritosQuery = await pool.request().query(`
      SELECT 
        ...
        CASE 
          WHEN COUNT(DISTINCT f.id_usuario) > 20 AND DATEDIFF(day, MAX(f.fecha_agregado), GETDATE()) < 7 
          THEN 1 ELSE 0 
        END as trending
      FROM Favoritos f
      INNER JOIN Cursos c ON f.id_curso = c.id_curso
      INNER JOIN Usuarios u ON c.id_instructor = u.id_usuario
      GROUP BY c.id_curso, c.titulo, c.imagen, c.id_categoria, u.nombre
...
// GET - Usuarios que guardaron un curso específico
router.get('/curso/:cursoId/usuarios', async (req, res) => {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT 
          ...
          u.email,
          f.fecha_agregado,
          CASE WHEN EXISTS (SELECT 1 FROM Compras WHERE id_usuario = u.id_usuario AND id_curso = @cursoId) 
               THEN 1 ELSE 0 END as comprado
        FROM Favoritos f
        INNER JOIN Usuarios u ON f.id_usuario = u.id_usuario
        WHERE f.id_curso = @cursoId
        ORDER BY f.fecha_agregado DESC
      `);
...
// POST - Enviar recomendaciones a un usuario
router.post('/usuario/:userId/recomendaciones', async (req, res) => {
  const { userId } = req.params;
  
  try {
    // Aquí implementarías la lógica de recomendaciones personalizadas
    // y el envío de email
    
    res.json({ 
      success: true, 
      message: 'Recomendaciones enviadas exitosamente' 
    });
  } catch (err) {
    console.error('Error al enviar recomendaciones:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});

// GET - Exportar reporte de favoritos
  ...
  try {
    ...
    const result = await pool.request().query(`
      SELECT 
        ...
        u.email,
        f.fecha_agregado,
        CASE WHEN EXISTS (SELECT 1 FROM Compras WHERE id_usuario = f.id_usuario AND id_curso = f.id_curso) 
             THEN 'Sí' ELSE 'No' END as comprado
      FROM Favoritos f
      INNER JOIN Cursos c ON f.id_curso = c.id_curso
      INNER JOIN Usuarios u ON f.id_usuario = u.id_usuario
      ORDER BY f.fecha_agregado DESC
    ...
    console.error('Error al exportar favoritos:', err);
    res.status(500).json({ success: false, error: err.message });
  }
});
